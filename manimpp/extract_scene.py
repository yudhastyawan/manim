...
from manimpp.utils.sounds import play_error_sound
from manimpp.utils.sounds import play_finish_sound
...
def prompt_user_for_choice(scene_classes):
    ...
    try:
        user_input = input(manimpp.constants.CHOOSE_NUMBER_MESSAGE)
        return [
            num_to_class[int(num_str)]
            for num_str in user_input.split(",")
        ]
    except KeyError:
        print(manimpp.constants.INVALID_NUMBER_MESSAGE)
        sys.exit(2)
        user_input = input(manimpp.constants.CHOOSE_NUMBER_MESSAGE)
        return [
            num_to_class[int(num_str)]
...
def get_scenes_to_render(scene_classes, config):
    if len(scene_classes) == 0:
        print(manimpp.constants.NO_SCENE_MESSAGE)
        return []
    if config["write_all"]:
    ...
    for scene_name in config["scene_names"]:
        ...
        if not found and (scene_name != ""):
            print(
                manimpp.constants.SCENE_NOT_FOUND_MESSAGE.format(
                    scene_name
                ),
                file=sys.stderr
            )
