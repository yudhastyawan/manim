...
class Anim3edit1(Scene):
  def construct(self):
    ...
    self.play(
      LaggedStart(*[
        # ApplyFunction(
        #   rect_func,recti,rate_func=there_and_back
        GrowFromCenter(recti
        ) for recti in rectgroup
      ],lag_ratio=0.001),
      AnimationGroup(
        Animation(Mobject(),run_time=2),
        Write(title),lag_ratio=1,
      )
    )
...
if __name__ == '__main__':
    YRender(Anim3edit1, active_preview=True)
